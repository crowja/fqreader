...
fqreader.o: fqreader.c fqreader.h
	$(CC) -c $(CPPFLAGS) $(CFLAGS) -o $@ fqreader.c

varstr.o: varstr.c varstr.h 
	$(CC) -c $(CPPFLAGS) $(CFLAGS) -o $@ varstr.c

check: fqreader.o varstr.o
	@for i in $(TESTS); \
	do \
	  echo "--------------------"; \
	  echo "Running test $$i ..."; \
	  ( $(CC)    $(CPPFLAGS) $(OTHER_INCLUDE) $(CFLAGS) $(OTHER_SOURCE) \
		-o t/a.out $$i.c fqreader.o varstr.o $(LDFLAGS) ) \
	  && ( t/a.out ); \
	done 

check-examples: fqreader.o varstr.o
	@for i in $(EXAMPLES); \
	do \
	  echo "--------------------"; \
	  echo "Running example $$i ..."; \
	  ( $(CC) -g $(CPPFLAGS) $(OTHER_INCLUDE) $(CFLAGS) $(OTHER_SOURCE) \
		-o ex/a.out $$i.c fqreader.o varstr.o $(LDFLAGS) ) \
	  && ( valgrind $(VALGRIND_FLAGS) ex/a.out < ex/SP1.fq); \
	done 

vcheck: fqreader.o varstr.o
	@for i in $(TESTS); \
	do \
	  echo "--------------------"; \
	  echo "Running test $$i ..."; \
...
echeck: fqreader.o varstr.o
	@for i in $(TESTS); \
	do \
	  echo "--------------------"; \
	  echo "Running test $$i ..."; \
	  ( $(CC)    $(CPPFLAGS) $(OTHER_INCLUDE) $(CFLAGS) $(OTHER_SOURCE) \
		-o t/a.out $$i.c fqreader.o varstr.o $(LDFLAGS_EFENCE) ) \
	  && ( LD_PRELOAD=libefence.so ./t/a.out ); \
	done 

indent: stamp
	@indent $(INDENT_FLAGS) fqreader.c
	@indent $(INDENT_FLAGS) fqreader.h
